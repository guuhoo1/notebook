...
<template>
  <aside v-if="authStore.isAuthenticated" class="hidden md:block fixed left-0 top-16 bottom-0 w-60 bg-surface-soft border-r border-hairline overflow-y-auto">
    <nav class="p-4">
      <div class="space-y-1">
        <button
          :class="[
      ...
      <div class="mt-6">
        ...
        <div v-if="categoryStore.loading" class="px-4 py-2 text-caption text-muted">
          加载中...
        </div>
        <div v-else-if="categoryStore.categories.length === 0" class="px-4 py-2 text-caption text-muted">
          暂无分类
        </div>
        <div v-else class="space-y-1">
          <button
            ...
            :class="[
              'w-full h-11 px-4 flex items-center rounded-md text-left transition-colors',
              activeItem === `category-${category.id}` ? 'bg-canvas text-ink' : 'text-body hover:bg-canvas/50'
            ]"
            @click="selectItem(`category-${category.id}`)"
          >
            <span
